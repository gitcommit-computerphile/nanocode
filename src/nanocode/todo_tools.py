...
from . import prompts
from .state import NanocodeState, Todo, format_todos
...
def recite_todos(
    ...
    """Re-inject the current plan before every model call.

    This is the structural half of recitation: the plan never has to survive in
    the message list, and the model never has to remember to re-read it.
    """
...
def make_compactor(context_window: int = DEFAULT_CONTEXT_WINDOW) -> AgentMiddleware:
    ...
    def compact_if_needed(state: NanocodeState, runtime: Any) -> dict[str, Any] | None:
        ...
        digest = SystemMessage(
            ...
            "The plan in `todos` and the record in `session_log` are current as of this "
            "point, and every file edit is already on disk. Re-read files if you need "
            "their contents.]"
        )
        return {
            "messages": [

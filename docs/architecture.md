...
- **`session_log` is append-only across the session**, so `session.json` holds
...
There is no `files` field — the disk already persists that. What remains is the
plan and a running log, which is enough to resume without a transcript.
...
is easy to forget on a long run. Nanocode re-injects the plan before every model
call from `recite_todos` middleware, so it cannot be forgotten and never has to
survive in the message list.
...
<project-root>/
  .nanocode/
    session.json                    todos + session_log, the resumable state
    logs/
...
`session.json` holds `todos`, `session_log`, `cwd`, and `updated_at` — never a
credential. API keys are read once from the environment at startup; `--resume`
expects the same environment variable to be set again, not a stored secret.
